...
func (s *OpeningRouter) InitOpeningRouter(Router *gin.RouterGroup, PublicRouter *gin.RouterGroup) {
	withoutAuth := PublicRouter.Group("open").Use(middleware.OpenCheck())
	{
		...
		withoutAuth.GET("mine/archives", mineArchivesApi.GetMineArchivesList)    // archives page
		withoutAuth.GET("mine/sentences", mineSentencesApi.GetMineSentencesList) // sentences page
		withoutAuth.GET("mine/resources", mineResourcesApi.SearchMineResources)  // search: books + blogs + projects + archives + sentences
