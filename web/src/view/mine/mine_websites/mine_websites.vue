    ...
    <div class="gva-search-box">
      ...
      >
        <el-form-item label="简称" prop="name">
          <el-input v-model="searchInfo.name" placeholder="简称" />
        </el-form-item>
      ...
      >
        ...
        <el-table-column align="center" label="图标" prop="icon">
          <template #default="{ row }">
            <custom-pic preview pic-type="file" :pic-src="row.icon" />
          </template>
        </el-table-column>

        <el-table-column align="center" label="简称" prop="name">
          <template #default="{ row }">
            <el-link :href="row.href" target="_blank">{{ row.name }}</el-link>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" label="操作" fixed="right" width="210">
          <template #default="{ row }">
            ...
            <el-button
              ...
              link
              icon="edit"
              @click="updateMineWebsitesFunc(row)"
            >
              编辑
            </el-button>
...
<script setup>
  import {
    createMineWebsites,
    deleteMineWebsites,
    deleteMineWebsitesByIds,
    updateMineWebsites,
    findMineWebsites,
    getMineWebsitesList
  } from '@/api/mine/mine_websites'
  ...
  const getTableData = async () => {
    const table = await getMineWebsitesList({
      page: page.value,
  ...
  const onDelete = async () => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
      ...
      const res = await deleteMineWebsitesByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
  ...
  // 更新行
  const updateMineWebsitesFunc = async (row) => {
    const res = await findMineWebsites({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
  ...
  // 删除行
  const deleteMineWebsitesFunc = async (row) => {
    const res = await deleteMineWebsites({ id: row.id })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功'
  ...
  const enterDialog = async () => {
    ...
    elFormRef.value?.validate(async (valid) => {
      ...
      switch (type.value) {
        case 'create':
          res = await createMineWebsites(formData.value)
          break
        case 'update':
          res = await updateMineWebsites(formData.value)
          break
        default:
          res = await createMineWebsites(formData.value)
          break
  ...
  const fetchCategoryList = async () => {
    const dataList = await getDictFunc('website_navigation_category')
